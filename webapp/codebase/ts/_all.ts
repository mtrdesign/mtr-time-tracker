///<reference path="../../typings/tsd.d.ts"/>

///<reference path="services/authentication.ts"/>
///<reference path="services/flash.ts"/>
///<reference path="services/page.ts"/>
///<reference path="services/profiles.ts"/>
///<reference path="services/projects.ts"/>
///<reference path="services/time_reports.ts"/>
///<reference path="services/users.ts"/>

///<reference path="controllers/404.ts"/>
///<reference path="controllers/account.ts"/>
///<reference path="controllers/home.ts"/>
///<reference path="controllers/login.ts"/>
///<reference path="controllers/page.ts"/>
///<reference path="controllers/project.ts"/>
///<reference path="controllers/time_report_edit.ts"/>
///<reference path="controllers/time_report_list.ts"/>
///<reference path="controllers/time_report_new.ts"/>
///<reference path="controllers/time_report_view.ts"/>

///<reference path="filters.ts"/>
///<reference path="http_loader.ts"/>
///<reference path="interface.ts"/>
///<reference path="init.ts"/>